...
class mosqpp_EXPORT mosquittopp {
	...
	public:
		...
		std::function<void(int, int)> OnConnectWithFlags;
		std::function<void(int)> OnDisconnect;
		std::function<void(int)> OnPublish;
		std::function<void(const struct mosquitto_message * )> OnMessage;
		std::function<void(int, int, const int *)> OnSubscribe;
		std::function<void(int)> OnUnsubscribe;
		std::function<void(int, const std::string&)> OnLog;
